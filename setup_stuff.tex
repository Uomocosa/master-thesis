% --------------------------------------------------------
% for PDF/A-1b pdf file - +++ DO NOT REMOVE THIS +++
% https://en.wikipedia.org/wiki/PDF/A
\PassOptionsToPackage{hidelinks}{hyperref}
\usepackage[a-1b]{pdfx}

% --------------------------------------------------------
% set document margins and line spacing
\usepackage[top=45mm,bottom=35mm,inner=35mm,outer=25mm]{geometry}
\usepackage{setspace}
\setstretch{1.6}
% --------------------------------------------------------
% mathematical environment
\usepackage{amsmath}
\usepackage{amsfonts}
\usepackage{amssymb}
\usepackage{amsthm}
% --------------------------------------------------------
% bio-chemical environment
\usepackage{mhchem}
% --------------------------------------------------------
% figures, graphics, captions and table rows
\usepackage{graphicx}
\usepackage[justification=centering,font={small,singlespacing}]{caption}
\renewcommand{\arraystretch}{1.5}

% --------------------------------------------------------
% references, citations
\usepackage{cite}

% --------------------------------------------------------
% other newer stuff!!!
\usepackage{booktabs}
\usepackage{microtype}
\usepackage[T1]{fontenc}
\usepackage{lmodern}
\usepackage{footnote}

% --------------------------------------------------------
% headers and footers
\usepackage{fancyhdr}
\setlength{\headheight}{14.5pt}
\setlength{\footskip}{16mm}


\fancypagestyle{default_page}{
	\fancyhead[C]{}
	\if@twoside%
		\fancyhead[LE,RO]{\sl CHAPTER \thechapter}
		\fancyhead[RE,LO]{}
	\else
		\fancyhead[R]{\sl CHAPTER \thechapter}
		\fancyhead[L]{}
	\fi
	\fancyfoot[L,R]{}
	\fancyfoot[C]{\small\thepage}
	\renewcommand{\headrulewidth}{0.1mm}
}

\fancypagestyle{empty_page}{
	\fancyhf{}
	\renewcommand{\headrulewidth}{0pt}
	\renewcommand{\footrulewidth}{0pt}
}



\fancypagestyle{final_stuff}{
	\fancyhead[C]{}
	\if@twoside%
		\fancyhead[LE,RO]{\sl \leftmark}
		\fancyhead[RE,LO]{}
	\else
		\fancyhead[R]{\sl \leftmark}
		\fancyhead[L]{}
	\fi
	\fancyfoot[L,R]{}
	\fancyfoot[C]{{\small\thepage}}
	\renewcommand{\headrulewidth}{0.1mm}
}

\makeatletter
\def\emptypage@emptypage{%
	\thispagestyle{empty}
	% \hbox{}%
	\vspace*{\fill}
	\begin{center}
		{\footnotesize This page intentionally left blank.}
	\end{center}
	\vspace{\fill}
	\newpage%
}%
\def\cleardoublepage{%
	\clearpage%
	\if@twoside%
		\ifodd\c@page%
			% do nothing
		\else%
			\emptypage@emptypage%

		\fi%
	\fi%
}%
\makeatother


% --------------------------------------------------------
% handle links and urls
\usepackage{url}

% --------------------------------------------------------
% code listing
\usepackage{listings}

% --------------------------------------------------------
% revision highlighting (changes w.r.t. the previously reviewed draft)
% Wrap changed/added inline text in \new{...} to mark it for the reviewer.
% NOTE: soul's \hl breaks if it wraps \cite, \footnote, \ref or math;
%       keep those OUTSIDE the \new{...} span.
\usepackage{xcolor}
\usepackage{soul}
\sethlcolor{yellow}
\newcommand{\new}[1]{\hl{#1}}
