\chapter{Predictive Modeling of Absorption Capacity}
\label{sec:predictive_chapter}

Having established the ability to generate novel polymer structures, this chapter addresses the complementary challenge of predicting their effectiveness. The central predictive task is to estimate the absorption capacity of a given polymer-molecule pair under specified environmental conditions. This chapter describes the experimental methodology, hyperparameter optimization, and results obtained using the Multi-Layer Perceptron architecture introduced in Chapter 2.

\section{Experimental Setup for the MLP}
\label{sec:mlp_setup}

The Multi-Layer Perceptron employed for capacity prediction is a feedforward neural network with a carefully chosen architecture designed to capture the nonlinear relationships between chemical features and absorption efficiency. The network consists of an input layer sized to match the dimensionality of the feature vector, multiple hidden layers with progressively decreasing widths (specifically: 16, 8, 4, 4, and 4 neurons in each respective hidden layer), and a single output neuron that produces the predicted capacity value.
...
K-Fold Cross-Validation, as described in Chapter 2, serves as the primary evaluation methodology. Rather than partitioning the data into a single train-test split, the dataset is divided into k equally sized folds. The model is trained k times, each time using k-1 folds for training and the remaining fold for validation. This approach ensures that every data point serves as both training and validation data exactly once, providing a comprehensive assessment of model performance.

Leave-One-Out Cross-Validation (LOOCV) represents the extreme case where k equals the number of data points. Each fold contains a single observation, and the model is trained on all remaining data. While computationally expensive, LOOCV is particularly valuable for small datasets because it maximizes the amount of training data available in each iteration and provides an almost unbiased estimate of generalization error.

In addition to cross-validation, a comprehensive hyperparameter search was conducted to identify the optimal model configuration. The search space included the learning rate, the number of training epochs, the batch size, and the choice of feature subsets. For each configuration, the Q-squared (Q²) statistic was computed from the cross-validation results. The Q² statistic, analogous to the R² coefficient in standard regression, measures the proportion of variance in the target variable that is explained by the model. A Q² value of 1 indicates perfect prediction, while a value of 0 indicates that the model performs no better than predicting the mean of the training data.
...
The hyperparameter optimization process evaluated a wide range of configurations using both standard K-fold and Leave-One-Out cross-validation. The results provide insight into the model's predictive capability under realistic data constraints.

Among the configurations tested, the optimal hyperparameters were identified based on the highest Q² score achieved during cross-validation. The best-performing configuration achieved a Q² score of 0.984 using LOOCV evaluation. This result is noteworthy given the data scarcity and the inherent complexity of polymer-molecule interactions. A Q² of 0.984 indicates that the model explains approximately 98.4\% of the variance in the absorption capacity, capturing a substantial portion of the underlying relationship.
...
It is important to interpret these results in context. The PDCC dataset contains observations from diverse polymer-molecule pairs under varying experimental conditions. The inherent heterogeneity of this dataset imposes an upper bound on achievable predictive performance. Even a perfect model could not achieve Q² = 1 if the experimental observations contain measurement noise or reflect genuine stochastic variation in the adsorption process. Nevertheless, a Q² of 0.984 demonstrates that machine learning can capture meaningful patterns in the polymer-molecule interaction space, providing utility for prioritizing candidates for experimental validation. It should be noted that these results were obtained on the augmented dataset, where linear interpolation and origin point addition were applied as described in Chapter 4; performance on the original, unaugmented data would be expected to be lower.

\begin{table}[htbp]
	\centering
	\caption{LOOCV results across MLP hyperparameter configurations. Top 10 experiments ranked by Q².}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{RESULTS/ahc_clustering/cluster_plot.png}
...
\section{Discussion of Model Limitations and Predictive Feasibility}
...
The second limitation concerns the representativeness of the training data. The scraped literature reports successful absorption experiments, potentially introducing publication bias. Polymer-molecule pairs that did not exhibit significant absorption may be underrepresented or absent from the dataset. A robust predictive model requires data on both successes and failures to learn the boundary between effective and ineffective absorbents.

The third limitation relates to feature quality. As discussed in Chapter 4, polymer features are computed from capped monomers rather than full polymer chains. This approximation neglects higher-order polymer properties such as chain length distribution, cross-linking density, and three-dimensional conformation, all of which influence absorption behavior. Future work incorporating more accurate polymer representations could improve predictive accuracy.

Despite these limitations, the predictive modeling framework demonstrates feasibility for the intended application. The model can provide reasonable estimates of absorption capacity for novel polymer-molecule pairs, enabling computational prioritization of candidates before laboratory synthesis and testing. The framework is extensible: as more experimental data become available, the model can be retrained to improve its accuracy and coverage.

\section{Polymer Filtering Based on Chemical Properties}
\label{sec:polymer_filtering}

Complementing the quantitative predictions of the MLP, a set of heuristic filters based on established chemical principles is applied to eliminate polymers that are unlikely to be effective absorbents. These filters encode domain knowledge about the requirements for successful adsorption in wastewater treatment.

The first filter concerns the partition coefficient (logP) of the polymer. For a polymer to function as an insoluble absorbent in aqueous solution, it must have a sufficiently high logP value to remain in a solid phase. A minimum logP threshold of 1.5 is applied, ensuring that the polymer remains hydrophobic enough to avoid dissolution while not being so lipophilic that it loses affinity for the aqueous pollutant.
...
The fourth filter addresses synthetic accessibility (SA score). A practical wastewater absorbent must be synthesizable at scale using reasonable chemical methods. The SA score, computed from structural features that affect synthetic difficulty, is constrained to a maximum value of 4.5 on a 1-10 scale.
...
\begin{figure}[htbp]
	...
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{RESULTS/predicted_capacities/plot_conc_aspirin.png}
		\caption{Predicted absorption capacity vs. initial concentration for the top-ranked polymer against aspirin.}
		\label{fig:pipeline_conc_aspirin}
	\end{minipage}\hfill
	\begin{minipage}{0.48\textwidth}
		\centering
		\includegraphics[width=\textwidth]{RESULTS/predicted_capacities/plot_ph_aspirin.png}
		\caption{Predicted absorption capacity vs. water pH for the top-ranked polymer against aspirin.}
