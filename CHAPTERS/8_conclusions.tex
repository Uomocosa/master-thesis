...
Third, a predictive model based on the Multi-Layer Perceptron was developed to estimate adsorption capacity from chemical features. Systematic hyperparameter optimization using Leave-One-Out Cross-Validation identified configurations achieving a Q² score of approximately 0.42. While data scarcity imposes fundamental limitations, this result demonstrates that meaningful predictive patterns can be extracted from the available data.

Fourth, a heuristic filtering framework based on established chemical principles was implemented. Filters based on logP, TPSA, FMO theory, and synthetic accessibility effectively eliminate candidates that are unlikely to be effective or practical absorbents. The integration of these filters with the predictive model provides a balanced approach that combines domain knowledge with data-driven prediction.
...
Several limitations of the current work must be acknowledged and addressed in future research.

Data scarcity remains the most critical challenge. The PDCC dataset contains only a few hundred experimental observations, orders of magnitude less than typical machine learning datasets. This scarcity limits the complexity of models that can be reliably trained and constrains the generalization capability of the predictive model. Future work should prioritize the creation of larger, standardized datasets through coordinated literature mining and experimental collaboration. Critically, data on failed adsorption experiments (where polymers showed little or no capacity for specific molecules) would be equally valuable as data on successes.

Publication bias in the training data may skew the model toward predicting positive outcomes. The scientific literature preferentially reports successful results, potentially leaving gaps in the coverage of the polymer-molecule interaction space. Efforts to collect negative results or to generate synthetic negative examples could help mitigate this bias.

The approximation of polymer features from capped monomers neglects higher-order structural properties. Real polymer adsorption behavior depends on chain length, molecular weight distribution, cross-linking density, and three-dimensional conformation. Future work incorporating more accurate polymer representations, such as molecular dynamics simulations of polymer chains, could improve feature quality and predictive accuracy.

The P-SMILES representation, while robustly supported by cheminformatics tools, may not capture all chemically relevant information. Extensions to alternative polymer notations, such as BigSMILES, could provide richer structural representations at the cost of reduced tool support and data availability.

Finally, the current work focuses exclusively on homopolymers. Many studies in the literature explore the use of dual-polymer systems and composite materials that may exhibit synergistic effects. Expanding the framework to handle multi-component materials would broaden its applicability to real-world wastewater treatment scenarios.
