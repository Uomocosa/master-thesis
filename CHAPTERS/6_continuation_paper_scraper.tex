...
The decisive limitation, therefore, is not the volume of scraped rows --- nor even their cleanliness --- but the number of \emph{distinct} polymer--molecule pairs available for testing. With only about seventy groups in the PDCC, roughly fourteen fall into each test split, far too few for a stable extrapolation estimate; the variance of $Q^2$ across seeds dwarfs any effect of the augmentation. This is what the per-source results really show: adding the small, hand-validated Claude Opus set did not help either --- for the compact architecture it moved $Q^2$ from about $-0.07$ to $-0.09$, the same seed-dependent wobble produced by the larger but noisier sources. On a benchmark this under-powered, a clean dataset and a noisy one are indistinguishable, because neither can be credited with a genuine improvement. The result is nonetheless useful: the paper-scraper succeeds at its narrow task of harvesting and structuring literature data, but no augmentation --- clean or bulk --- can buy generalisation while the evaluation rests on so few distinct pairs. The true prerequisite is a much larger and more diverse pool of polymer--molecule pairs, enough for $Q^2$ to stabilise; only then could one even tell whether better or cleaner data improves extrapolation --- and reaching that scale is where the automated pipeline, paired with manual revision, can help.
